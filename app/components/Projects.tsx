  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className={`font-mono text-[11px] uppercase tracking-widest ${accentText[p.accent]}`}>
            project · {p.slug}
          </div>
      ...
      <button
        onClick={() => setOpen((o) => !o)}
        className="mt-5 flex items-center gap-1 self-start rounded-md border border-bg-border px-3 py-1.5 text-xs font-mono text-ink-dim hover:text-ink hover:border-accent-cyan/40"
      >
        <ChevronDown size={12} className={`transition-transform ${open ? 'rotate-180' : ''}`} />
        {open ? 'collapse' : 'show architecture + approach'}
      </button>
